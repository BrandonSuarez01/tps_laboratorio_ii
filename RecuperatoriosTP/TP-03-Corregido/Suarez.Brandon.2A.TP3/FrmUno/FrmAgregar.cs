    ...
    public partial class FrmAgregar : Form, IValidar 
    {
        ...
        public void AgregarCLiente()
        {
            if (string.IsNullOrWhiteSpace(txtNombre.Text) || string.IsNullOrWhiteSpace(txtDni.Text)
                || string.IsNullOrWhiteSpace(datFecha.Text)
                || string.IsNullOrWhiteSpace(txtPrecio.Text) || string.IsNullOrWhiteSpace(chListServicios.Text)
                || string.IsNullOrWhiteSpace(txtDias.Text) || string.IsNullOrWhiteSpace(cmbEquipo.Text)) 
            {
            ...
            if (VerificarNumero(txtDni.Text) == true && VerificarLetra(txtNombre.Text) == false)
            {
                ...
                clienteNuevo.Fecha = datFecha.Text;
                clienteNuevo.ServicioElegido = new Servicios(txtPrecio.Text, chListServicios.Text, txtDias.Text, cmbEquipo.Text);
                this.DialogResult = DialogResult.OK;
            }
            else
            {
                MessageBox.Show("Complete el campo DNI e Importe con numeros y el campo nombre solo con letras.", "Alerta", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            }
        }
        private void btnPrecios_Click(object sender, EventArgs e)
        {
            MessageBox.Show(Servicios.MostrarPrecios(), "Lista de costos.", MessageBoxButtons.OK);
        }
    }
}
