...
namespace FrmUno
{
    public partial class FrmAgregar : Form, IValidar 
    {
        ...
        {
            if (string.IsNullOrWhiteSpace(txtNombre.Text) || string.IsNullOrWhiteSpace(txtDni.Text) || string.IsNullOrWhiteSpace(cmbProductos.Text) || string.IsNullOrWhiteSpace(datFecha.Text))
            {
                throw new NameException("Por favor complete todos los campos.");
            }
            if(VerificarNumero(txtDni.Text) == true && VerificarLetra(txtNombre.Text) == false && VerificarNumero(txtImporte.Text) == true)
            {
                clienteNuevo.Nombre = txtNombre.Text;
                clienteNuevo.Dni = txtDni.Text;
                clienteNuevo.Producto = cmbProductos.Text;
                clienteNuevo.Importe = txtImporte.Text;
